...
class StorageScheme(guts.Object):
    ...
    def save_carpets(
        ...
        for deltat, carpet_group in by_deltat.items():
            ...
            tmin_group = tmin if tmin is not None else \
                min(cp.tmin for cp in carpet_group)

            tmax_group = tmax if tmax is not None else \
                max(cp.tmax for cp in carpet_group) + deltat
