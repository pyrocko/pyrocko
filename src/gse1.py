...
def read_dat1_chk1(f, data_format, diff_flag, nsamples):
    ...
    if data_format == 'INTV' and diff_flag == 0:
        ...
        raise GSE1LoadError('GSE1 data format %s with differencing=%i not supported.' % (data_format, diff_flag))
...
def iload(filename, load_data=True):
    ...
    try:
        while True:
            h = read_wid1(f)
            tmin, nsamples, sta, chid, cha, sample_rate, _, data_format, diff_flag, gain = h[:10]
            deltat = 1.0/sample_rate
            if load_data:
                ydata, checksum = read_dat1_chk1(f, data_format, diff_flag, nsamples)
                tmax = None
            else:
            ...
            yield trace.Trace('', sta, '', cha, tmin=tmin, tmax=tmax, 
