...
/*
 * The lookup table is to 90DEG, the input can be -360 to 360 DEG, where
 * negative values are transformed to positive before further processing. We
 * need two additional bits (*4) to represent 360 DEG:
 */
...
int16_t sin1(int16_t angle) {
  ...
  v0 &= TABLE_MASK;
  v1 = sin90[v0] +
       (int16_t)(((int32_t)(sin90[v0 + 1] - sin90[v0]) * (v1 & INTERP_MASK)) >>
                 INTERP_BITS);
...
int16_t cos1(int16_t angle) {
  ...
  return sin1(angle - (270. / 360) * INT16_MAX);
}
