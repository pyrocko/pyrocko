...
from pyrocko.util import unpack_fixed, gmctime
...
class Waveform:
    def __init__(self, wid2, sta2, chk2, dat2):
        ...
        if dat2.rawdata is not None:
            if self.wid2.sub_format == 'CM6':
                ...
                self.data = gse2_ext.decode_m6(
                    ''.join(dat2.rawdata), wid2.samps)
            else:
                logger.error(
                    'cannot load GSE sub format "%s" (not implemented)'
                    % self.wid2.sub_format)
                self.data = None
        ...
        if self.data is None:
            self.tmax = self.wid2.tmin \
                + 1.0/self.wid2.samprate * (wid2.samps - 1)
        else:
            self.tmax = None
    ...
    def __str__(self):
        return ' '.join([
            ...
            self.wid2.auxid,
            self.wid2.sub_format,
            gmctime(self.wid2.tmin)])

    def trace(self):
        return trace.Trace(
            station=self.wid2.station,
            location=self.wid2.auxid,
            channel=self.wid2.channel,
            tmin=self.wid2.tmin,
            tmax=self.tmax,
...
class Station:
    ...
    def __init__(self, network, station, type, lat, lon, coordsys, elevation,
                 ondate, offdate):
...
class Channel:
    def __init__(self, network, station, channel, auxid, lat, lon, coordsys,
                 elevation, depth, hang, vang, samplerate, instrument, ondate,
                 offdate):
    ...
    def __str__(self):
        # as GSE2.1 format
        return (
            '%-9s %-5s %-3s %-4s %9.5f %10.5f %-12s %5.3f %5.3f %6.1f '
            '%5.1f %11.6f %-6s %-10s %-10s' % (
                self.network, self.station, self.channel, self.auxid, self.lat,
                self.lon, self.coordinate_system, self.elevation, self.depth,
...
class DataSection:
    ...
    def interprete(self, load_data=True):
        mapping = {
            'error_log': self.interprete_error_log,
        ...
        if self.data_type in mapping:
            ...
            logger.warn('Skipping unimplemented GSE data type "%s"'
                        % self.data_type)
    ...
    def interprete_station(self, load_data=True):
        if self.version not in ('GSE2.0', 'GSE2.1'):
            logger.error(
                'Can not interprete GSE station information of version %s'
                % self.version)
        ...
        if len(self.data) < 1:
            raise BadGSESection(
                'Need at least header line for station section')

        header = self.data[0].strip()
        if (self.version == 'GSE2.0' and header.lower().split() !=
                'Sta Type Latitude Longitude '
                'Elev On Date Off Date'.lower().split()):
            logger.warn('GSE station section header line does not match what '
                        'is expected for GSE version 2.0')

        if (self.version == 'GSE2.1' and header.lower().split() !=
                'Net Sta Type Latitude Longitude Coord Sys Elev On Date Off '
                'Date'.lower().split()):
            logger.warn('GSE station section header line does not match what '
                        'is expected for GSE version 2.1')

        for line in self.data[1:]:
            if self.version == 'GSE2.1':
                net, sta, typ, lat, lon, coordsys, elev, ondate, offdate = \
                    unpack_fixed(
                        'a9,x1,a5,x1,a4,x1,f9,x1,f10,x1,a12,x1,f5,x1,a10,x1,'
                        'a10', line)
            elif self.version == 'GSE2.0':
                sta, typ, lat, lon, elev, ondate, offdate = unpack_fixed(
                    'a5,x1,a4,x1,f9,x1,f10,x1,f7,x1,a10,x1,a10', line)
                net, coordsys = '', ''
    ...
    def interprete_channel(self, load_data=True):
        if self.version not in ('GSE2.0', 'GSE2.1'):
            logger.error(
                'Can not interprete GSE channel information of version %s'
                % self.version)
            return

        if len(self.data) < 1:
            raise BadGSESection(
                'Need at least header line for channel section')

        header = self.data[0].strip()
        if self.version == 'GSE2.0' and header.lower().split() != 'Sta Chan '\
                'Aux Latitude Longitude Elev Depth Hang Vang Sample_Rate '\
                'Inst On Date Off Date'.lower().split():
            logger.warn('GSE channel section header line does not match what '
                        'is expected for GSE version 2.0')

        if self.version == 'GSE2.1' and header.lower().split() != 'Net Sta '\
                'Chan Aux Latitude Longitude Coord Sys Elev Depth Hang Vang '\
                'Sample Rate Inst On Date Off Date'.lower().split():
            logger.warn('GSE channel section header line does not match what '
                        'is expected for GSE version 2.1')

        for line in self.data[1:]:
            if self.version == 'GSE2.1':
                net, sta, cha, auxid, lat, lon, coordsys, elev, depth, hang, \
                    vang, samprate, instype, ondate, offdate = unpack_fixed(
                        'a9,x1,a5,x1,a3,x1,a4,x1,f9,x1,f10,x1,a12,x1,f5,x1,f5,'
                        'x1,f6,x1,f5,x1,f11,x1,a6,x1,a10,x1,a10', line)
                ondate = slashdate(ondate)
                offdate = slashdate(offdate)

            elif self.version == 'GSE2.0':
                sta, cha, auxid, lat, lon, elev, depth, hang, vang, \
                    samprate, instype, ondate, offdate = unpack_fixed(
                        'a5,x1,a3,x1,a4,x1,f9,x1,f10,x1,f7,x1,f6,x1,f6,x1,f5,'
                        'x1,f11,x1,a7,x1,a10,x1,a10', line)
                net, coordsys = '', ''

                if offdate.find(':') != -1:
                    # hack for some non-standard gse2.0 files from norsar
            ...
            yield Channel(
                net, sta, cha, auxid, lat, lon, coordsys, elev, depth, hang,
                vang, samprate, instype, ondate, offdate)
        ...
        for line in self.data:
            ...
            if at in (0, 2):
                if line.startswith('WID2 '):
                    ...
                    wid2.tmin = (calendar.timegm(time.strptime(
                        ...
                        '%Y/%m/%d %H:%M:%S')) + float(line[24:28]))

                    strtmin, wid2.station, wid2.channel, wid2.auxid, \
                        wid2.sub_format, wid2.samps, wid2.samprate, \
                        wid2.calib, wid2.calper, wid2.instype, wid2.hang, \
                        wid2.vang = unpack_fixed(
                            'x5,a23,x1,a5,x1,a3,x1,a4,x1,a3,x1,i8,x1,f11,x1,'
                            'f10,x1,f7,x1,a6,x1,f5,x1,f4', line)
            ...
            if at == 1:
                if line.startswith('STA2'):
                    sta2 = Anon()
                    sta2.network, sta2.lat, sta2.lon, sta2.coordsys, \
                        sta2.elev, sta2.depth = unpack_fixed(
