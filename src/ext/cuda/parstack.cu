 ...
 public:
  ...
  ParstackCUDAKernel(size_t narrays, size_t nshifts, size_t nsamples,
      ...
      : ParstackCUDAKernel(narrays, nshifts, nsamples, lengthout, offsetout,
                           arrays, weights, result, lengths, offsets, shifts, 0,
                           1, 32, 256, err) {}
  ...
  ParstackCUDAKernel(T **arrays, parstack_kernel_arguments_t<T> args,
      ...
      : ParstackCUDAKernel(args.narrays, args.nshifts, args.nsamples,
                           args.lengthout, args.offsetout, arrays, args.weights,
                           args.result, args.lengths, args.offsets, args.shifts,
                           method, 1, args.work_per_thread, 256, err) {}
  ...
  __host__ virtual void launch_kernel(dim3 _grid, dim3 _blocks,
                                      ...
                                      parstack_kernel_arguments_t<T> _args) = 0;
...
#pragma omp parallel for private(i) num_threads(nparallel)
    ...
    cudaCheck(cudaMemset2D(args.result, args.pitchout, 0, lengthout * sizeof(T),
                           nshifts),
              "result: memset failed", err);

    cudaCheck(cudaMalloc((void **)&(args.offsets), narrays * sizeof(int32_t)),
              "offsets: allocation failed", err);

    cudaCheck(cudaMalloc((void **)&(args.lengths), narrays * sizeof(size_t)),
    ...
    cudaCheck(cudaMemcpyAsync(args.lengths, lengths, narrays * sizeof(size_t),
                              cudaMemcpyHostToDevice, stream),
    ...
    if (method == 0) {
      cudaCheck(cudaMemcpy2DAsync(args.result, args.pitchout, result,
    ...
    if (method == 1) {
      ...
      cudaCheck(cudaMemcpy2D(result, lengthout * sizeof(T), args.result,
                             args.pitchout, lengthout * sizeof(T), nshifts,
                             cudaMemcpyDeviceToHost),
...
__global__ void parstack_atomic_kernel(parstack_kernel_arguments_t<T> args) {
  ...
  if (shift_idx < args.nshifts && arr_idx < args.narrays &&
    ...
    int32_t istart =
        args.offsets[arr_idx] + args.shifts[shift_idx * args.narrays + arr_idx];
    if (weight != 0.0) {
      ...
      size_t end = max((size_t)0, min(args.lengthout - istart + args.offsetout,
...
class AtomicParstackCUDAKernel : public ParstackCUDAKernel<T> {
 ...
 public:
  ...
  __host__ void launch_kernel(dim3 _grid, dim3 _blocks, size_t _shared_memory,
                              unsigned int _block_size,
                              parstack_kernel_arguments_t<T> _args) {
    select_parstack_kernel_for_block_size(parstack_atomic_kernel, _block_size);
  }

  __host__ bool is_compatible() { return CUDADevice::properties()->major >= 6; }
