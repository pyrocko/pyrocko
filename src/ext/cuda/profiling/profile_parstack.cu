...
int profile(size_t narrays, size_t nshifts, size_t nsamples,
    ...
    char *err_msg = NULL;
    int err =
        cuda_parstack(narrays, arrays, offsets, lengths, nshifts, shifts,
                      weights, method, lengthout, offsetout, result, IMPL_CUDA,
                      nparallel, target_block_threads, &err_msg);
    ...
    for (size_t i = 0; i < narrays; i++) free(arrays[i]);
    free(arrays);
    free(offsets);
    free(lengths);
    free(shifts);
