...
int good_array(PyObject* o, int typenum_want, npy_intp size_want, int ndim_want,
               npy_intp* shape_want, char* name) {
  int i;

  if (!PyArray_Check(o)) {
    PyErr_Format(PyExc_AttributeError, "%s not a NumPy array", name);
    return 0;
  }

  if (PyArray_TYPE((PyArrayObject*)o) != typenum_want) {
    PyErr_Format(PyExc_AttributeError, "array %s of unexpected type", name);
    return 0;
  }
  ...
  if (size_want != -1 && size_want != PyArray_SIZE((PyArrayObject*)o)) {
    PyErr_Format(PyExc_AttributeError, "array %s is of unexpected size", name);
    return 0;
  }
  if (ndim_want != -1 && ndim_want != PyArray_NDIM((PyArrayObject*)o)) {
    PyErr_Format(PyExc_AttributeError, "array %s is of unexpected ndim", name);
    return 0;
  }
  ...
  if (ndim_want != -1 && shape_want != NULL) {
    for (i = 0; i < ndim_want; i++) {
      if (shape_want[i] != -1 &&
        ...
        PyErr_Format(PyExc_AttributeError, "array %s is of unexpected shape",
                     name);
...
static void heap_down(heap_t* heap, size_t index, double* keys,
                      size_t* backpointers) {
  size_t v, w;
  v = index;
...
static void heap_up(heap_t* heap, size_t index, double* keys,
                    size_t* backpointers) {
  size_t v, u;
  v = index;
  while (v > 0) {
...
static eikonal_error_t heap_push(heap_t* heap, size_t index, double* keys,
                                 size_t* backpointers) {
  if (heap->n + 1 > heap->nmax) return HEAP_FULL;
...
static void heap_update(heap_t* heap, size_t index, double newkey, double* keys,
                        size_t* backpointers) {
  double oldkey;
...
static eikonal_error_t update_neighbor(size_t index, double* speeds,
                                       size_t ndim, size_t* shape, double delta,
                                       double* times, size_t* backpointers,
                                       heap_t* heap, double tref) {
  double s1, s2, tnew, d;
  ...
  for (idim = ndim; idim-- > 0;) {
    ...
    if (ix + 1 < shape[idim])
      tmins[idim] = dmin(tmins[idim], (backpointers[index + stride] == ALIVE)
                                          ? times[index + stride]
                                          : THUGE);
  ...
  retval = SUCCESS;
  d = s1 * s1 -
      ndim_eff * (s2 - delta * delta / (speeds[index] * speeds[index]));
  if (d >= 0) {
...
static eikonal_error_t update_neighbors(size_t index, double* speeds,
                                        size_t ndim, size_t* shape,
                                        double delta, double* times,
                                        size_t* backpointers, heap_t* heap) {
  size_t idim;
  size_t stride;
  size_t ix;
  ...
  for (idim = ndim; idim-- > 0;) {
    ...
    if (1 <= ix) {
      retval_temp = update_neighbor(index - stride, speeds, ndim, shape, delta,
                                    times, backpointers, heap, times[index]);
...
static eikonal_error_t eikonal_solver_fmm_cartesian(double* speeds, size_t ndim,
                                                    size_t* shape, double delta,
                                                    double* times) {
  heap_t* heap;
  size_t idim;
  size_t n, i;
  ...
  for (index = 0; index < n; index++) {
    if (backpointers[index] == ALIVE) {
      retval_temp = update_neighbors(index, speeds, ndim, shape, delta, times,
                                     backpointers, heap);
...
static PyObject* w_eikonal_solver_fmm_cartesian(PyObject* m, PyObject* args,
  ...
  static char* kwlist[] = {"speeds", "times", "delta", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwds, "OOd", kwlist, &speeds_arr,
                                   &times_arr, &delta)) {
    return NULL;
  }

  if (!good_array(speeds_arr, NPY_FLOAT64, -1, -1, NULL, "speeds")) return NULL;
  ...
  Py_BEGIN_ALLOW_THREADS err = eikonal_solver_fmm_cartesian(
      speeds, (size_t)ndim, size_t_shape, delta, times);
  Py_END_ALLOW_THREADS if (SUCCESS != err) {
...
PyMODINIT_FUNC PyInit_eikonal_ext(void)

{
  ...
  st->error =
      PyErr_NewException("pyrocko.eikonal_ext.EikonalExtError", NULL, NULL);
