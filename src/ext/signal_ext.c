...
const char* signal_error_names[] = {
    ...
    "antidrift: time correction slope too extreme (must be in range [0.9, "
    "1.1])",
...
static int64_t imin(int64_t i, int64_t j) { return i < j ? i : j; }
static int64_t imax(int64_t i, int64_t j) { return i > j ? i : j; }

static signal_error_t antidrift(int64_t n_control,
                                const int64_t* indices_control,
                                const double* t_control, int64_t n_in,
                                const double* samples_in, int64_t n_out,
                                double tmin_out, double deltat_out,
                                double* samples_out) {
  double* coeffs = NULL;
...
#pragma omp simd
    ...
    for (icoeff = 0; icoeff < icoeff_lo; icoeff++) {
      samples_out[i] += samples_in[0] * coeffs[ientry * ncoeffs + icoeff];
    }

    for (icoeff = icoeff_hi; icoeff < ncoeffs; icoeff++) {
      samples_out[i] +=
          samples_in[n_in - 1] * coeffs[ientry * ncoeffs + icoeff];
    }
  }
...
static PyObject* w_antidrift(PyObject* m, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "OOOddO", &arr_indices_control, &arr_t_control,
                        &arr_samples_in, &tmin_out, &deltat_out,
                        &arr_samples_out)) {
  ...
  if (n_control != PyArray_SIZE((PyArrayObject*)arr_t_control)) {
    PyErr_SetString(st->error, "sizes of indices_control and t_control differ");
    return NULL;
  }
  ...
  Py_BEGIN_ALLOW_THREADS err =
      antidrift(n_control, indices_control, t_control, n_in, samples_in, n_out,
                tmin_out, deltat_out, samples_out);
...
static PyObject* w_apply_costaper(PyObject* m, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "ddddOdd", &xa, &xb, &xc, &xd, &y_arr, &x0,
                        &dx)) {
    PyErr_SetString(st->error,
...
static PyMethodDef signal_ext_methods[] = {
    ...
    {"apply_costaper", w_apply_costaper, METH_VARARGS, "apply cosine taper"},
...
PyMODINIT_FUNC PyInit_signal_ext(void)
