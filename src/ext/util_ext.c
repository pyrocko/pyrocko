...
const char* util_error_names[] = {
    "SUCCESS",       "ALLOC_FAILED",  "TIME_FORMAT_ERROR",
    "MKTIME_FAILED", "GMTIME_FAILED",
};
    ...
    *strrchr(format2, '.') = '\0'; /* cannot fail here */
    ...
    if (sfrac != NULL) {
      if (nexpect > 0 && strlen(sfrac) != (size_t)(nexpect + 1)) {
        return TIME_FORMAT_ERROR; /* incorrect number of digits in fractional
                                     seconds part */
      }
      errno = 0;
      *tfrac = strtod(sfrac, NULL);
      if (errno != 0) {
        return TIME_FORMAT_ERROR; /* could not convert fractional part to a
                                     number */
      }
      *sfrac = '\0';
    }
...
util_error_t stt_c_locale(const char* s, const char* format, time_t* t,
                          double* tfrac) {
  char* saved_locale;
  util_error_t err;
...
util_error_t tts_c_locale(time_t t, double tfrac, const char* format,
                          char** sout) {
  char* saved_locale;
  util_error_t err;
...
static PyMethodDef util_ext_methods[] = {
    {"tts", w_tts, METH_VARARGS, "time to string"},

    {"stt", w_stt, METH_VARARGS, "string to time"},
...
PyMODINIT_FUNC PyInit_util_ext(void)
