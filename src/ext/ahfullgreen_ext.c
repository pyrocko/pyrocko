...
static ahfullgreen_error_t numpy_or_none_to_c_double(PyObject* o,
                                                     ssize_t size_want,
                                                     double** arr,
                                                     size_t* size) {
  if (o == Py_None) {
...
static ahfullgreen_error_t numpy_or_none_to_c_complex(PyObject* o,
                                                      ssize_t size_want,
                                                      complex_t** arr,
                                                      size_t* size) {
  if (o == Py_None) {
...
static ahfullgreen_error_t add_seismogram(
    double vp, double vs, double density, double qp, double qs, double* x,
    double* f, double* m6,
    int out_quantity,  // 1: velocity, 2: acceleration
    double out_delta, double out_offset, size_t out_size, complex_t* out_x,
    complex_t* out_y, complex_t* out_z, int want_far, int want_intermediate,
    int want_near) {
  ...
  for (i = 0; i < out_size; i++) {
    ...
    if (i != 0) {
      b2[i] = mulcr(mulcc(dfactor, cexp(mulcr(iw, -r / vp))),
                    exp(-w * r / (2.0 * vp * qp)));
      b3[i] = mulcr(mulcc(dfactor, cexp(mulcr(iw, -r / vs))),
                    exp(-w * r / (2.0 * vs * qs)));
      b1[i] = subcc(mulcc(addcr(iwi, r / vp), mulcc(b2[i], iwi)),
                    mulcc(addcr(iwi, r / vs), mulcc(b3[i], iwi)));
    } else {
  ...
  for (n = 0; n < 3; n++) {
    ...
    for (p = 0; p < 3; p++) {
      for (q = 0; q < 3; q++) {
        if (want_near) {
          a1 = (15. * gamma[n] * gamma[p] * gamma[q] -
                3. * (gamma[n] * (p == q) + gamma[p] * (n == q) +
                      gamma[q] * (n == p))) /
               (density4pi * r4);
        } else {
          a1 = 0.;
        ...
        if (want_intermediate) {
          a2 = (6. * gamma[n] * gamma[p] * gamma[q] - gamma[n] * (p == q) -
                gamma[p] * (n == q) - gamma[q] * (n == p)) /
               (density4pi * vp2 * r2);

          a3 = -(6. * gamma[n] * gamma[p] * gamma[q] - gamma[n] * (p == q) -
                 gamma[p] * (n == q) - 2. * gamma[q] * (n == p)) /
               (density4pi * vs2 * r2);
        } else {
          a2 = a3 = 0.;
        }
        ...
        if (want_far) {
          a4 = (gamma[n] * gamma[p] * gamma[q]) / (density4pi * vp3 * r);
        ...
        for (i = 0; i < out_size; i++) {
          ...
          out[n][i] = addcc(
              out[n][i],
              mulcr(addcc(addcc(addcc(addcc(mulcr(b1[i], a1), mulcr(b2[i], a2)),
                                      mulcr(b3[i], a3)),
    ...
    for (p = 0; p < 3; p++) {
      a6 = (3. * gamma[n] * gamma[p] - (n == p)) / (density4pi * r2 * r);

      a7 = (gamma[n] * gamma[p]) / (density4pi * vp2 * r);

      a8 = -(gamma[n] * gamma[p] - (n == p)) / (density4pi * vs2 * r);

      for (i = 0; i < out_size; i++) {
        /*out[n][i] += (a6*b1[i] + a7*b2[i] + a8*b3[i]) * f[p];*/
        out[n][i] = addcc(out[n][i],
                          mulcr(addcc(addcc(mulcr(b1[i], a6), mulcr(b2[i], a7)),
                                      mulcr(b3[i], a8)),
                                f[p]));
      }
    }
...
static PyObject* w_add_seismogram(PyObject* m, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "dddddOOOiddOOOiii", &vp, &vs, &density, &qp, &qs,
                        &x_arr, &f_arr, &m6_arr, &out_quantity, &out_delta,
                        &out_offset, &out_x_arr, &out_y_arr, &out_z_arr,
  ...
  if (SUCCESS != numpy_or_none_to_c_complex(out_x_arr, -1, &out_x, &out_x_size))
    return NULL;
  if (SUCCESS != numpy_or_none_to_c_complex(out_y_arr, -1, &out_y, &out_y_size))
    return NULL;
  if (SUCCESS != numpy_or_none_to_c_complex(out_z_arr, -1, &out_z, &out_z_size))
    return NULL;
  ...
  if (SUCCESS != numpy_or_none_to_c_double(x_arr, 3, &x, &dummy_size))
    return NULL;
  if (SUCCESS != numpy_or_none_to_c_double(f_arr, 3, &f, &dummy_size))
    return NULL;
  if (SUCCESS != numpy_or_none_to_c_double(m6_arr, 6, &m6, &dummy_size))
    return NULL;

  err = add_seismogram(vp, vs, density, qp, qs, x, f, m6, out_quantity,
                       out_delta, out_offset, out_size, out_x, out_y, out_z,
                       want_far, want_intermediate, want_near);
...
PyMODINIT_FUNC PyInit_ahfullgreen_ext(void)
