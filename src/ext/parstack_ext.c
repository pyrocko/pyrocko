...
int parstack(size_t narrays, double** arrays, int32_t* offsets, size_t* lengths,
             size_t nshifts, int32_t* shifts, double* weights, int method,
             size_t lengthout, int32_t offsetout, double* result,
             int nparallel);

int parstackf(size_t narrays, float** arrays, int32_t* offsets, size_t* lengths,
              size_t nshifts, int32_t* shifts, float* weights, int method,
              size_t lengthout, int32_t offsetout, float* result,
              int nparallel);

static int32_t i32min(int32_t a, int32_t b) { return (a < b) ? a : b; }

static size_t smin(size_t a, size_t b) { return (a < b) ? a : b; }
...
#pragma omp simd
#endif
          for (i = (size_t)i32max(0, imin - istart);
               i < (size_t)i32max(
                       0, i32min(nsamp - istart + imin, lengths[iarray]));
               i++) {
...
int fargmax(float* arrayin, uint32_t* arrayout, size_t nx, size_t ny,
            int nparallel) {
  size_t ix, iy, ix_offset, imax[NBLOCK];
  float vmax[NBLOCK];
  (void)nparallel;
...
int bad_array(PyObject* o, int type_num, char* name) {
  ...
  if (!PyArray_ISCARRAY((PyArrayObject*)o)) {
    PyErr_Format(PyExc_ValueError, "%s is not contiguous or not well behaved",
                 name);
...
static PyObject* w_parstack(PyObject* module, PyObject* args, PyObject* kwds) {
  ...
  static char* kwlist[] = {"arrays",    "offsets",   "shifts",    "weights",
                           "method",    "lengthout", "offsetout", "result",
                           "nparallel", NULL};

  if (!PyArg_ParseTupleAndKeywords(
          args, kwds, "OOOOiii|Oi", kwlist, &arrays, &offsets, &shifts,
  ...
  if (dtype != NPY_FLOAT && dtype != NPY_DOUBLE) {
    PyErr_SetString(PyExc_ValueError,
                    "Bad dtype, only float64 and float32 is supported.");
    return NULL;
  }
  ...
  Py_BEGIN_ALLOW_THREADS if (dtype == NPY_FLOAT) {
    err = parstackf(narrays, (float**)carrays, coffsets, clengths, nshifts,
                    cshifts, PyArray_DATA((PyArrayObject*)weights), method,
                    lengthout, offsetout, (float*)cresult, nparallel);
  }
  else if (dtype == NPY_DOUBLE) {
    err = parstack(narrays, (double**)carrays, coffsets, clengths, nshifts,
                   cshifts, PyArray_DATA((PyArrayObject*)weights), method,
                   lengthout, offsetout, (double*)cresult, nparallel);
  }
...
static PyObject* w_argmax(PyObject* module, PyObject* args, PyObject* kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|i", kwlist, &arrayin,
                                   &nparallel))
    return NULL;
  ...
  Py_BEGIN_ALLOW_THREADS if (dtype == NPY_FLOAT) {
    err = fargmax((float*)carrayin, cresult, (size_t)shape[1], (size_t)shape[0],
                  nparallel);
  }
  else { /* if (dtype == NPY_DOUBLE) { */
...
static PyMethodDef ParstackMethods[] = {
    {"parstack", (PyCFunction)(void (*)(void))w_parstack,
     METH_VARARGS | METH_KEYWORDS, "Parallel weight-and-delay stacking"},

    {"argmax", (PyCFunction)(void (*)(void))w_argmax,
     METH_VARARGS | METH_KEYWORDS, "argmax of 2D numpy array along axis=0"},
...
PyMODINIT_FUNC PyInit_parstack_ext(void)

{
  ...
  st->error =
      PyErr_NewException("pyrocko.parstack_ext.ParstackExtError", NULL, NULL);
  if (st->error == NULL) {
    Py_DECREF(module);
    INITERROR;
  }
