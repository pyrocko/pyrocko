...
int good_array(PyObject* arr, int typenum, ssize_t size_want, int ndim_want,
               npy_intp* shape_want) {
  int i;
  ...
  if (ndim_want != -1) {
    for (i = 0; i < ndim_want; i++) {
      if (shape_want[i] != -1 &&
          shape_want[i] != PyArray_DIMS((PyArrayObject*)arr)[i]) {
...
static float64_t sqr(float64_t x) { return x * x; }
...
static void azibazi(float64_t alat, float64_t alon, float64_t blat,
  ...
  if (alat == blat && alon == blon) {
    ...
    *azi = R2D *
           atan2(cos(D2R * alat) * cos(D2R * blat) * sin(D2R * (blon - alon)),
                 sin(D2R * blat) - sin(D2R * alat) * cd);
    *bazi = R2D *
            atan2(cos(D2R * blat) * cos(D2R * alat) * sin(D2R * (alon - blon)),
                  sin(D2R * alat) - sin(D2R * blat) * cd);
  }
}

static void azibazi_array(float64_t* alats, float64_t* alons, float64_t* blats,
                          float64_t* blons, npy_intp size, float64_t* azis,
                          float64_t* bazis) {
  npy_intp i;
  for (i = 0; i < size; i++) {
...
float64_t east, float64_t *lat_new, float64_t *lon_new) { float64_t a, b, c,
gamma, alphasign, alpha;
    ...
    alpha = cos(a) - cos(b)*cos(c) < 0.0 ? (alpha > 0.0 ? M_PI-alpha :
-M_PI-alpha) : alpha;
...
static void azibazi4(float64_t *a, float64_t *b, float64_t *azi, float64_t
*bazi) {
...
int check_latlon_ranges(float64_t alat, float64_t alon, float64_t blat,
                        float64_t blon) {
  if (alat > 90. || alat < -90. || blat > 90. || blat < -90.) {
    PyErr_SetString(
        PyExc_ValueError,
        "distance_accurate50m: Latitude must be between -90 and 90 degree.");
    return 0;
  }
  if (alon > 180. || alon < -180. || blon > 180. || blon < -180.) {
    PyErr_SetString(
        PyExc_ValueError,
        "distance_accurate50m: Longitude must be between -180 and 180 degree.");
    return 0;
  }
...
static void distance_accurate50m_array(float64_t* alats, float64_t* alons,
                                       float64_t* blats, float64_t* blons,
                                       npy_intp npairs, float64_t* dists) {
  npy_intp i;
  for (i = 0; i < npairs; i++) {
...
static PyObject* w_distance_accurate50m(PyObject* m, PyObject* args) {
  ...
  Py_BEGIN_ALLOW_THREADS distance_accurate50m(alat, alon, blat, blon, &dist);
  Py_END_ALLOW_THREADS return Py_BuildValue("d", dist);
}

static PyObject* w_distance_accurate50m_numpy(PyObject* m, PyObject* args) {
  PyObject *alats_arr, *alons_arr, *blats_arr, *blons_arr;
  PyArrayObject *c_alats_arr, *c_alons_arr, *c_blats_arr, *c_blons_arr,
      *dists_arr;
  float64_t *alats, *alons, *blats, *blons;
  npy_intp size[1];
  struct module_state* st = GETSTATE(m);

  if (!PyArg_ParseTuple(args, "OOOO", &alats_arr, &alons_arr, &blats_arr,
                        &blons_arr)) {
  ...
  Py_BEGIN_ALLOW_THREADS distance_accurate50m_array(
      alats, alons, blats, blons, size[0], PyArray_DATA(dists_arr));
...
static PyObject* w_azibazi_numpy(PyObject* m, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "OOOO", &alats_arr, &alons_arr, &blats_arr,
    ...
    PyErr_SetString(
        st->error,
        "usage: azibazi_numpy(alats, blats, alons, blons) -> (azis, bazis)");
    return NULL;
  }
  ...
  Py_BEGIN_ALLOW_THREADS azibazi_array(alats, alons, blats, blons, size[0],
                                       PyArray_DATA(azis_arr),
                                       PyArray_DATA(bazis_arr));
  Py_END_ALLOW_THREADS
...
:type blon: float"},
    ...
    {"distance_accurate50m_numpy", (PyCFunction)w_distance_accurate50m_numpy,
     METH_VARARGS,
...
PyMODINIT_FUNC PyInit_orthodrome_ext(void)

{
  ...
  st->error = PyErr_NewException("pyrocko.orthodrome_ext.OrthodromeExtError",
                                 NULL, NULL);
  if (st->error == NULL) {
    Py_DECREF(module);
    INITERROR;
