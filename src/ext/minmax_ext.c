...
#pragma omp for schedule(dynamic, 1) nowait
        ...
        for (iout = 0; iout < iout_size; iout += NBLOCK) {
            for (iout_offset = 0;
                ...
                idx[iout_offset] = 0;
                vals[iout_offset] =
                    (type == ARGMAX || type == MAX) ? DBL_MIN : DBL_MAX;
            }
            for (irdc = 0; irdc < rdc_size; irdc++) {
                for (iout_offset = 0;
                     iout_offset <
                     (size_t)min((size_t)NBLOCK, iout_size - iout);
                     iout_offset++) {
                    double val;
                    if (axis == 1)
                    ...
                    int equal =
                        (val == vals[iout_offset]) && (irdc < idx[iout_offset]);
...
static PyObject *w_minmax_kernel_parameters(PyObject *module, PyObject *args,
    ...
    static char *kwlist[] = {"cols",
                             ...
                             NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "nn|iinnn", kwlist, &cols,
                                     &rows, &axis, &impl, &target_block_threads,
                                     &work_per_thread,
                                     &shared_memory_per_thread)) {
...
static PyObject *w_minmax(PyObject *module, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|iiiin", kwlist, &arrayin,
                                     ...
                                     &target_block_threads)) {
        PyErr_SetString(st->error,
                        "usage minmax(array, typ, axis, nparallel, "
                        "impl, target_block_threads)");
        return NULL;
    }

    if (!good_array("array", arrayin, NPY_DOUBLE)) return NULL;
    ...
    if (axis != 0 && axis != 1) {
        PyErr_SetString(st->error,
                        "array axis for 2D array must be either 0 or 1");
        return NULL;
    }
    ...
    if (max_idx >= (size_t)UINT32_MAX) {
        PyErr_SetString(st->error,
                        "indices of the dimension to be aggregated must be "
                        "smaller than 2^32");
        return NULL;
    }
    ...
    Py_BEGIN_ALLOW_THREADS if (impl == IMPL_CUDA || impl == IMPL_CUDA_THRUST ||
        ...
        if (!check_cuda_supported()) {
            ...
            RET_CODE =
                cuda_minmax(carrayin, cvresult, ciresult, axis, type, cols,
        ...
        RET_CODE = minmax(carrayin, ciresult, cvresult, axis, cols, rows,
                          nparallel, type);
    }
