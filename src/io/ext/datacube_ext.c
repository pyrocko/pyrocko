...
locale_t uselocale(locale_t new_locale) {
  ...
  if (new_locale == LC_GLOBAL_LOCALE) {
    ...
    for (int i = LC_MIN; i <= LC_MAX; i++) setlocale(i, "C");
...
datacube_error_t parse_gps_xpv_mpv_header(char* header, double* temp,
...
datacube_error_t parse_gps_rpv_header(char* header, double* temp, double* lat,
                                      double* lon, double* elevation) {
...
datacube_error_t bookmark_array_append(bookmark_array_t* arr, size_t ipos,
                                       off_t fpos) {
  bookmark_t *p, *el;
  size_t n;
  if (arr->fill == arr->size) {
...
datacube_error_t datacube_read(reader_t* reader, size_t n) {
  ...
  while (n > 0) {
    ...
    memcpy(reader->buf + reader->buf_fill, reader->buf_1 + reader->rpos, n1);
...
datacube_error_t datacube_read_to(reader_t* reader, char sepmin,
                                  char* sepfound) {
  datacube_error_t err;
  while (1) {
    ...
    if ((unsigned int)reader->buf[reader->buf_fill - 1] >=
        (unsigned int)sepmin) {
...
datacube_error_t datacube_read_gps_block(reader_t* reader) {
  ...
  if (7 != sscanf(b, "%2d%2d%2d%3d%2d%2d%4d", &tm.tm_hour, &tm.tm_min,
                  &tm.tm_sec, &msecs, &tm.tm_mday, &tm.tm_mon, &tm.tm_year)) {
    return BAD_GPS_BLOCK;
  }
  ...
  b += 17;
  b[6] = '\0';
  if (4 != sscanf(b, "%2i%1i%2i%1i", &gps_utc_time_offset, &current_fix_source,
                  &number_usable_svs, &gps_utc_offset_flag)) {
    return BAD_GPS_BLOCK;
  }
...
datacube_error_t datacube_load(reader_t* reader) {
   ...
   * 0xc0  192  12: Event block from 1 byte info, from version 5.0(1C) 2.0(3C) 2
   * bytes if first byte is 1 should abort (buffer overrun in recorder) 0xcf 12:
   * diagnostics x byte ??? read while (byte >> 4) < 8 0xd0  208  13: info block
   * ascii ??? 0xd1       13:  aux channel read 1 byte -> (byte & 0xf) - 2 is
   * number of bytes to read additionally 0xe0  224  14: end block 0xef : header
  ...
  while (1) {
    ...
    if (blocktype == 8) {
      ...
      fprintf(stderr, "skipping block type %d at offset %lld\n", blocktype,
              (long long int)datacube_tell(reader));
      err = BAD_GPS_BLOCK;
    ...
    if (jumpallowed && reader->gps_tags.fill == N_GPS_TAGS_WANTED) {
      ...
      if (gps_on == 0) { /* cycled GPS */
        ...
        roffset = (off_t)(toffset * 1.0 / reader->deltat *
                              (reader->nchannels * 4 + 1) +
                          nblocks_needed * (gps_ti * 60) * 80);
...
static PyObject* transfer_arrays(reader_t* reader) {
  ...
  if (!isnull(reader->arrays)) {
    for (i = 0; i < reader->nchannels; i++) {
      ...
      Py_BEGIN_ALLOW_THREADS memcpy(PyArray_DATA((PyArrayObject*)array),
                                    reader->arrays[i].elements,
                                    reader->arrays[i].fill * sizeof(int32_t));
      Py_END_ALLOW_THREADS free(reader->arrays[i].elements);
...
static PyObject* gps_tags_to_pytup(reader_t* reader) {
  ...
  for (i = 0; i < n; i++) {
    ((int64_t*)PyArray_DATA((PyArrayObject*)aipos))[i] =
        reader->gps_tags.elements[i].ipos;
    ((double*)PyArray_DATA((PyArrayObject*)at))[i] =
        reader->gps_tags.elements[i].t;
    ((int8_t*)PyArray_DATA((PyArrayObject*)afix))[i] =
        reader->gps_tags.elements[i].fix;
    ((int8_t*)PyArray_DATA((PyArrayObject*)ansvs))[i] =
        reader->gps_tags.elements[i].nsvs;

    ((double*)PyArray_DATA((PyArrayObject*)lats))[i] =
        reader->gps_tags.elements[i].lat;
    ((double*)PyArray_DATA((PyArrayObject*)lons))[i] =
        reader->gps_tags.elements[i].lon;
    ((double*)PyArray_DATA((PyArrayObject*)elevations))[i] =
...
static PyObject* bookmarks_to_pyarray(reader_t* reader) {
  ...
  for (i = 0; i < n; i++) {
    ((int64_t*)PyArray_DATA((PyArrayObject*)out))[i * 2] =
        reader->bookmarks.elements[i].ipos;
    ((int64_t*)PyArray_DATA((PyArrayObject*)out))[i * 2 + 1] =
        reader->bookmarks.elements[i].fpos;
  }
  return out;
}
...
int pyarray_to_bookmarks(reader_t* reader, PyObject* barr) {
  ...
  for (i = 0; i < n; i++) {
    bookmark_array_append(&reader->bookmarks, (size_t)carr[i * 2],
                          (off_t)carr[i * 2 + 1]);
  }
  return 0;
}
...
static PyObject* w_datacube_load(PyObject* m, PyObject* args) {
  ...
  */
  ...
  if (!PyArg_ParseTuple(args, "iinnO", &f, &load_data, &offset_want,
                        &nsamples_want, &barr)) {
  ...
  Py_BEGIN_ALLOW_THREADS err = datacube_load(&reader);
  Py_END_ALLOW_THREADS if (err != SUCCESS) {
    PyErr_SetString(st->error, datacube_error_names[err]);
    uselocale(oldloc);
    freelocale(loc);
    return NULL;
...
static PyMethodDef datacube_ext_methods[] = {
    {"load", w_datacube_load, METH_VARARGS, "load raw datacube file"},
...
PyMODINIT_FUNC PyInit_datacube_ext(void)

{
  ...
  st->error =
      PyErr_NewException("pyrocko.datacube_ext.DataCubeError", NULL, NULL);
  if (st->error == NULL) {
    Py_DECREF(module);
    INITERROR;
  }
