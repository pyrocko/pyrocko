...
static PyObject *mseed_get_traces(PyObject *m, PyObject *args, PyObject *kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|OLikk", kwlist, &filename,
                                   &unpackdata, &offset_temp,
                                   &segment_size_temp, &_tmin, &_tmax))
    return NULL;
  ...
  while (mst) {
    ...
    if (unpackdata == Py_True) {
      if (mst->datasamples == NULL) {
        if (mst->numsamples != 0) {
          PyErr_Format(st->error, "Data curruption: `datasamples` is NULL but "
...
static PyObject *mseed_bytes(PyObject *m, PyObject *args, PyObject *kwds) {
  ...
  for (itr = 0; itr < PySequence_Length(in_traces); itr++) {
    ...
    Py_BEGIN_ALLOW_THREADS;
    mst_pack(mst, &copy_memory, (void *)&mem_info, record_length, msdetype, 1,
             &psamples, 1, 0, NULL);
    mst_free(&mst);
    Py_END_ALLOW_THREADS;
