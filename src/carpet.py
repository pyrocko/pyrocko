...
logger = logging.getLogger('pyrocko.carpet')
...
class CarpetStats(Object):
    ...
    @classmethod
    def make(cls, data):
        data = num.where(num.isfinite(data), data, num.nan)
        nan_count = num.sum(num.isnan(data))
...
class Carpet(Object):
    ...
    :param traces:
        If given, construct multi-trace carpet from given single-component
        waveforms (see :py:func:`~pyrocko.trace.get_traces_data_as_array`) and
    ...
    def summary(self):
        ...
        return util.fmt_summary(
            self.summary_entries, (10, 20, 5, 7, 10, 10, 25, 25, 50))
    ...
    def fill_template(self, template, **additional):
        if '{' in template:
            ...
            return template % CarpetStringFiller(self, additional=additional)
        else:
            return template
    ...
    def overlaps(self, *args):
        '''
        Check this carpet has overlap with a given time span or other carpet.
        '''

        if len(args) == 1 and isinstance(args[0], Carpet):
            tmin, tmax = args[0].tmin, args[0].tmax
        elif len(args) == 2:
            tmin, tmax = args
    ...
    def resample_band(
        ...
        values_out[values_out == 0.0] = num.nan
...
def join(carpets):
    ...
    for group in groups:
        if len(group) > 1:
            data = num.hstack([cp.data for cp in group])
