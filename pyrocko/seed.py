...
blockette_definitions_dict = dict( [ (v.btype, v) for v in blockette_definitions ] )

def blockette_definition(key):
    if not key in blockette_definitions_dict:
...
class DatalessSeedReader:
    ...
    def read(self, filename):
        ...
        sequence_number, type_code, continuation_code = self._unpack_control_header(data)
        blockette_type, blockette_length = self._peek_into_blockette(data[8:8+7])
        ...
        while True:
            try:
                blockette_type, blockette_length = self._peek_into_blockette(data[pos:pos+7])
            ...
            while len(data) < pos+blockette_length:
                ...
                sequence_number, type_code, continuation_code = self._unpack_control_header(moredata)
                assert continuation_code == '*'
                data += moredata[8:]
