...
# A record consists of a record header a record payload, and possibly padding.
# A record header consists of a label, a version, the record size, the payload size, a hash, and a record type.
...
def pack_value(type, value):
    try:
        ...
        raise FileError('Packing value failed (type=%s, value=%s, error=%s).' % (type,str(value)[:500], e))
    
def unpack_value(type, value):
    try:
        return castings[type][1](value)
    except Exception, e:
        raise FileError('Unpacking value failed (type=%s, error=%s).' % (type, e))
...
class Record:
    ...
    def __init__(self, parent, mode, size_record, size_payload, hash, type, format, do_hash):
        self.mode = mode
        ...
        if mode == 'w':
            ...
            self.size_padding = self.size_record - size_record_header - self.size_payload
    ...
    def close(self):
        ...
        if self.mode == 'r':
            if self._hasher and self._hasher.digest() != self.hash:
                ...
                raise FileError('Hash computed from record data does not match value given in header.')
            else:
            ...
            if self.size_record is not None and self.size_payload > self.size_record - size_record_header:
                raise FileError('Too much data to fit into size-limited record.')
            
            if self.size_record is None:
                self.size_record = self.size_payload + size_record_header
            
            self.size_padding = self.size_record - self.size_payload - size_record_header
               
            if self._hasher is not None:
                self.hash = self._hasher.digest()
            
            
            self._parent.write_record_header(self.size_record, self.size_payload, self.hash, self.type)
...
class File:
    ...
    def read_record_header(self):
        ...
        label, version, size_record, size_payload, hash, type = unpack('>4s4sQQ20s20s', data)
    ...
    def write_record_header(self, size_record, size_payload, hash, type):
        if hash is None:
            hash = no_hash
        data = pack('>4s4sQQ20s20s', self._file_type_label,
             self._file_version, size_record, size_payload, hash, type.ljust(20)[:20])
        self._f.write(data)
    ...
    def next_record(self, check_hash=False):
        ...
        self._current_record = Record(self, 'r', size_record, size_payload, hash, type, format, check_hash)
    ...
    def add_record(self, type, size_record=None, make_hash=False):
        ...
        format = self._record_formats[type]
        self._current_record = Record(self, 'w', size_record, 0, None, type, format, make_hash)
        return self._current_record
