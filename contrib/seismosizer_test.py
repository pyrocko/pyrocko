...
class RectangularSource(DCSource):
    ...
    def __init__(self, **kwargs):
        if 'slip' in kwargs and 'magnitude' in kwargs:
            print 'Slip and Magnitude defined: Slip will be taken'
    ...
    def moment_to_slip(self):
        return self.update(slip=float(self.moment/(self.width*self.length*self.shearm)))
...
class LocalEngine(Engine):
    ...
    def calculate_model_prediction_sensitivity(self, *args, **kwargs):
        '''Calculate the model prediction Covariance Sensitivity Kernel.
           ...
           Following Duputel et al. 2014
            ...
            :Input: source_parms = list of parameters with respect to which the kernel 
                           is being calculated e.g. ['strike', 'dip', 'depth']
            !!!NEEDS to have seismosizer source object parameter variable name convention!!!
                (see seismosizer.source.keys() dictionary)

            calculate_model_prediction_sensitivity(request, source_params, **kwargs)
            calculate_model_prediction_sensitivity(sources, targets, source_params, **kwargs)
        ...
        if len(args) == 2:
            ...
            kwargs.update(Request.args2kwargs(args[0:1]))
        ...
        for ref_source in request.sources:
            ...
            for param in source_params:
                ...
                setattr(calc_source_p2h, param, ref_source[param] + (2*h))
                setattr(calc_source_ph, param, ref_source[param] + (h))
                setattr(calc_source_mh, param, ref_source[param] - (h))
                setattr(calc_source_m2h, param, ref_source[param] - (2*h))

                calc_sources = [calc_source_p2h, calc_source_ph, calc_source_mh, calc_source_m2h]

                response = self.process(sources= calc_sources, targets = request.targets, nprocs=nprocs)

                res_p2h = response.results_list[0]   # results list len of number of sources
                res_ph = response.results_list[1]
                res_mh = response.results_list[2]
                res_m2h = response.results_list[3]

                for k in xrange(len(request.targets)):
                    # calculate numerical partial derivative for each source and target
                    sensitivity_param_list[par_count][k] = sensitivity_param_list[par_count][k] + (-res_p2h[k].trace.data + 8*res_ph[k].trace.data
                                        -8*res_mh[k].trace.data + res_m2h[k].trace.data)/(12*h)
        ...
        for param in source_params:
            for k in xrange(len(request.targets)):
                sensitivity_param_trcs[par_count][k] = trace.Trace(
                                        ...
                                        deltat = res_p2h[k].trace.deltat,
                                        tmin = res_p2h[k].trace.tmin,
